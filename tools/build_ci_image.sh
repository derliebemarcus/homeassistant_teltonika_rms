...
image="registry.home.siczb.de/siczb/teltonika-rms-ci:${BUILD_NUMBER}"
...
RUN python3 -m pip install --upgrade pip \
  --index-url https://artifacts.home.siczb.de/repository/pypi-proxy/simple/
...
podman build --pull=never -t "${image}" -f Dockerfile.ci .
